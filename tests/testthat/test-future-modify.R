...
furrr_test_that("future_modify() default method works", {
  ...
  expect_identical(
    future_modify(data.frame(x = 1, y = 2), ~3),
    data.frame(x = 3, y = 3)
  )
})
...
furrr_test_that("future_modify_at() default works", {
  ...
  expect_identical(
    future_modify_at(data.frame(x = 1, y = 2), 2, ~3),
    data.frame(x = 1, y = 3)
  )
})
...
furrr_test_that("future_modify_at() variants works", {
  ...
  expect_identical(
    future_modify_at(c("a", "b", "c"), c(1, 3), toupper),
    c("A", "b", "C")
  )
  expect_identical(
    future_modify_at(c(TRUE, FALSE, TRUE), c(1, 3), ~NA),
    c(NA, FALSE, NA)
  )
})
